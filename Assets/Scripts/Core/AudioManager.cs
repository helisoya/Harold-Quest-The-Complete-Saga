...
public class AudioManager : MonoBehaviour
{
    ...
    public void PlaySFX(AudioClip effect, float volume = 1f, float pitch = 1f){
        ...
        source.pitch = pitch;
        source.outputAudioMixerGroup = AudioMixerManager.instance.gsfx;
        source.Play();
    ...
    public class SONG{
        ...
        public SONG(AudioClip clip,float _maxVolume, float pitch, float startingVolume, bool playOnStart, bool loop){
            source = AudioManager.instance.CreateNewSource(string.Format("SONG [{0}]",clip.name));
            source.clip = clip;
            source.volume = startingVolume;
            maxVolume = _maxVolume;
            source.pitch = pitch;
            source.loop = loop;
            source.outputAudioMixerGroup = AudioMixerManager.instance.gbgm;

            AudioManager.instance.allSongs.Add(this);
        ...
        public void DestroySong(){
            AudioManager.instance.allSongs.Remove(this);
